...
const (
	clusterCronJobCreater = "kubernetes"
	eGroup                = "group"
...
func HandleClusterResourceEvent() {
	//可以考虑ufleet创建的cronjob直接绑定k8s cronjob,
	//在k8s cronjob更新的时候再更新绑定的k8s  cronjob.
	//在删除的,标记底层资源已经被移除.

	for {
		pe := <-cluster.CronJobEventChan
		log.DebugPrint("recieve cluster cronjob event : %v", pe)
		...
		go func(e cluster.Event) {
			...
			case cluster.ActionDelete:
				//忽略ufleet主动创建的资源
				if e.FromUfleet {
					log.DebugPrint("cronjob event %v from ufleet,ignore ", e)
					return
				}
				...
				if err != nil {
					log.ErrorPrint("cluster cronjob  event handler/delete:", err)
					return
				}
			...
			case cluster.ActionCreate:
				...
				if !ok {
					log.ErrorPrint("handle cluster cronjob event fail: group \"%v\" not exist", e.Group)
					return
				}

				workspace, ok := group.Workspaces[e.Workspace]
				if !ok {
					log.ErrorPrint("handle cluster cronjob event fail: workspace \"%v\" not exist", e.Workspace)
					return
				}
				...
				p.Group = e.Group
				p.User = clusterCronJobCreater
				workspace.CronJobs[e.Name] = p
	...
	case backend.ActionDelete:
		...
		case eResource:
			...
			if e.Resource != nil {
				...
				delete(workspace.CronJobs, *e.Resource)
				group.Workspaces[*e.Workspace] = workspace
				rm.Groups[e.Group] = group
				return
			}
