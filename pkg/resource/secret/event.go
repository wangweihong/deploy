...
const (
	clusterSecretCreater = "kubernetes"
	eGroup               = "group"
	...
	for {
		...
		go func(e cluster.Event) {
			...
			case cluster.ActionCreate:
				...
				p.Group = e.Group
				p.User = clusterSecretCreater
				workspace.Secrets[e.Name] = p
	...
	case backend.ActionDelete:
		...
		case eResource:
			...
			if e.Resource != nil {
				...
				delete(workspace.Secrets, *e.Resource)
				group.Workspaces[*e.Workspace] = workspace
				rm.Groups[e.Group] = group
				return
			}
