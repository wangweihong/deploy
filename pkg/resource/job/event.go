...
func HandleClusterResourceEvent() {
	//可以考虑ufleet创建的job直接绑定k8s job,
	//在k8s job更新的时候再更新绑定的k8s  job.
	//在删除的,标记底层资源已经被移除.

	for {
		pe := <-cluster.JobEventChan
		log.DebugPrint("recieve cluster job event : %v", pe)
		...
		go func(e cluster.Event) {
			...
			case cluster.ActionDelete:
				//忽略ufleet主动创建的资源
				if e.FromUfleet {
					log.DebugPrint("job event %v from ufleet,ignore ", e)
					return
				}
				...
				if err != nil {
					log.ErrorPrint("cluster job  event handler/delete:", err)
					return
				}
			...
			case cluster.ActionCreate:
				...
				if !ok {
					log.ErrorPrint("handle cluster job event fail: group \"%v\" not exist", e.Group)
					return
				}

				workspace, ok := group.Workspaces[e.Workspace]
				if !ok {
					log.ErrorPrint("handle cluster job event fail: workspace \"%v\" not exist", e.Workspace)
					return
				}
