...
const (
	clusterServiceAccountCreater = "kubernetes"
	eGroup                       = "group"
...
func HandleClusterResourceEvent() {
	//可以考虑ufleet创建的serviceaccount直接绑定k8s serviceaccount,
	//在k8s serviceaccount更新的时候再更新绑定的k8s  serviceaccount.
	//在删除的,标记底层资源已经被移除.

	for {
		pe := <-cluster.ServiceAccountEventChan
		log.DebugPrint("recieve cluster serviceaccount event : %v", pe)
		...
		go func(e cluster.Event) {
			...
			case cluster.ActionDelete:
				//忽略ufleet主动创建的资源
				if e.FromUfleet {
					log.DebugPrint("serviceaccount event %v from ufleet,ignore ", e)
					return
				}
				...
				if err != nil {
					log.ErrorPrint("cluster serviceaccount  event handler/delete:", err)
					return
				}
			...
			case cluster.ActionCreate:
				...
				if !ok {
					log.ErrorPrint("handle cluster serviceaccount event fail: group \"%v\" not exist", e.Group)
					return
				}

				workspace, ok := group.Workspaces[e.Workspace]
				if !ok {
					log.ErrorPrint("handle cluster serviceaccount event fail: workspace \"%v\" not exist", e.Workspace)
					return
				}

				x, ok := workspace.ServiceAccounts[e.Name]
				if ok {
					if x.memoryOnly {
						log.ErrorPrint("handle cluster serviceaccount create event fail: serviceaccount\"%v\" has exist", e.Name)
					}
					return
				}

				var p ServiceAccount
				p.Name = e.Name
				p.memoryOnly = true
				p.Workspace = e.Workspace
				p.Group = e.Group
				p.User = clusterServiceAccountCreater
				workspace.ServiceAccounts[e.Name] = p
...
func EventHandler(e backend.ResourceEvent) {
	...
	case backend.ActionDelete:
		...
		case eResource:
			...
			if e.Resource != nil {
				...
				delete(workspace.ServiceAccounts, *e.Resource)
				group.Workspaces[*e.Workspace] = workspace
				rm.Groups[e.Group] = group
				return
			}
	...
	case backend.ActionAdd, backend.ActionCreate, backend.ActionUpdate:
		...
		case eResource:
			...
			ws.ServiceAccounts[*e.Resource] = app
			group.Workspaces[*e.Workspace] = ws
			rm.Groups[e.Group] = group
			//这是一个工作区事件
		}
