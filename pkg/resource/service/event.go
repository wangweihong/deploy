...
const (
	clusterServiceCreater = "kubernetes"
	eGroup                = "group"
	...
	for {
		...
		go func(e cluster.Event) {
			...
			case cluster.ActionCreate:
				...
				_, ok = workspace.Services[e.Name]
				if ok {
				...
				p.Group = e.Group
				p.User = clusterServiceCreater
				workspace.Services[e.Name] = p
	...
	case backend.ActionDelete:
		...
		case eResource:
			...
			if e.Resource != nil {
				...
				delete(workspace.Services, *e.Resource)
				group.Workspaces[*e.Workspace] = workspace
				rm.Groups[e.Group] = group
				return
			}
